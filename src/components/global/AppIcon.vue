...
<template>
  <el-icon class="AppIcon-root" v-bind="iconPropsResult">
    <!-- 插槽透传 -->
    <component :is="name" v-if="!$slots.default" class="icon-component"></component>
...
<script lang="ts" setup name="AppIcon">
  ...
  const props = defineProps({
    // 组件属性
    iconProps: { type: Object, default() { return {}; } },
    // 组件名，字符串或组件
...
<style scoped lang="scss">
  .AppIcon-root{ align-self: center;
    .icon-component{ vertical-align: middle;
      :hover{ opacity: 0.9; }
    }
  }
</style>
