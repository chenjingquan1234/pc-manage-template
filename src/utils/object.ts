const objUtil = { assign, deepAssign, isEmpty, notEmpty, isObject };
 ...
 * @returns {*}
 */
function assign(target = {}, ...sources: any[]) {
 ...
 * @param sources {any[]} 数据源。一个或多个对象
 */
function deepAssign(target: Record<string, any> = {}, ...sources: any[]) {
  for (const source of sources) {
    for (const [key, desc] of Object.entries(
      Object.getOwnPropertyDescriptors(source)
    )) {
 ...
 *  @returns {boolean}
 */
function isEmpty(value: Record<string, any>) {
 ...
 * @returns {boolean}
 */
function notEmpty(obj: Record<string, any>) {
